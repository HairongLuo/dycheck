    ...
    *,
    ...
    for img0, img1, mask0, mask1 in common.tqdm(data_loader, desc=desc):
        ...
        def _compute_flow(x0, x1):
            # Ensure tensors are contiguous to prevent potential cuDNN errors.
...
def main(_):
    ...
    split = "val"
    data_dir = osp.join(config.data_dir, config.sequence)
    covisible_dir = osp.join(
        data_dir, "covisible", f"{config.factor}x", split
    )
    ...
    # load frame_names
    with open(osp.join(data_dir, "splits", "train.json"), "r") as f:
        json_data = json.load(f)
        support_frame_names = sorted(json_data["frame_names"])
    with open(osp.join(data_dir, "splits", "val.json"), "r") as f:
        json_data = json.load(f)
        frame_names = sorted(json_data["frame_names"])

    # load imgs
    def load_rgba(rgb_path: str) -> np.ndarray:
    ...
    imgs, masks = np.split(
        np.array(
            [
                load_rgba(osp.join(data_dir, "rgb", "1x", name + ".png")) 
                for name 
                in frame_names
            ]
        ),
